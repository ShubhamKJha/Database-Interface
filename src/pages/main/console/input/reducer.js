export default function inputReducer(state, action) {
    ...
    switch(action.type) {
        ...
        case "console/input/MOVE_HISTORY_CURSOR": {
            ...
            // zero and historyLength. Zero represents the cursor content 
            // entered in the console by the user, not a history entry; the
            // first history entry is at 1
            const nextScrollback = Math.min(
                Math.max(
                    0, state.consoleInput.consoleScrollbackPosition + 
